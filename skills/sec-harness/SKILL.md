...
T1. **Tier-1 substrate** (no LLM) — `python -m sec_harness.graph build --target <T> --workspace <WS> --sha <sha>`; structural_index + ast-grep nodes/edges + osv/secrets/crypto facts → `kb/graph.json` v1, consumed by recon, architecture, and threat-model.
